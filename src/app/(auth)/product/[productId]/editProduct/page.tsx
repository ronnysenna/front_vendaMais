...
const N8N_GET_PRODUCT_WEBHOOK_URL =
  "https://n8n.ronnysenna.com.br/webhook/buscar-produto-por-id"; // Substitua pelo seu URL real

// **SEU WEBHOOK N8N PARA ATUALIZAR PRODUTO**
// Este webhook deve receber productId e os dados atualizados do produto
const N8N_UPDATE_PRODUCT_WEBHOOK_URL =
  "https://n8n.ronnysenna.com.br/webhook/atualizar-produto"; // Substitua pelo seu URL real
...
export default function EditProductPage() {
      ...
      try {
        ...
        if (data && data.product) {
          setProduct({
            ...
            variations: Array.isArray(data.product.variations)
              ? data.product.variations.join(", ")
              : "",
        ...
        setError(
          `Não foi possível carregar o produto. Erro: ${err instanceof Error ? err.message : String(err)}`,
        );
        setProduct(null);
      } finally {
  ...
  const handleValueOrStockChange = (
    e: React.ChangeEvent<HTMLInputElement>,
    field: "value" | "stockQuantity",
  ) => {
    const value = e.target.value;
    if (/^\d*\.?\d*$/.test(value) || value === "") {
      // Para value, permite ponto decimal
      setProduct((prev) => (prev ? { ...prev, [field]: value } : null));
    } else if (
      (field === "stockQuantity" && /^\d*$/.test(value)) ||
      value === ""
    ) {
      // Para stockQuantity, apenas dígitos
      setProduct((prev) => (prev ? { ...prev, [field]: value } : null));
    }
  };
  ...
  const handleImageUpload = async (): Promise<string | null> => {
    ...
    try {
      ...
      setErrorMessage(
        `Erro no upload da imagem: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!product) {
      // Não deveria acontecer se o produto for carregado
    ...
    if (
      ...
      !product.variations.toString().trim() ||
    ...
    try {
      const response = await fetch(N8N_UPDATE_PRODUCT_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          ...
          variations: product.variations
            .toString()
            .split(",")
            .map((v) => v.trim()), // Garante que variações seja um array
      ...
      setErrorMessage(
        `Não foi possível atualizar o produto. Erro: ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  if (errorMessage && !product) {
    // Mostra erro se não conseguiu carregar o produto
    return (
      <div className="max-w-xl mx-auto mt-10 text-white text-center">
        <p className="text-red-500 text-lg">{errorMessage}</p>
        <p className="text-gray-400 mt-2">
          Verifique o ID do produto ou tente novamente.
        </p>
      </div>
    );
  }
  ...
  if (!product) {
    // Caso não encontre o produto e não haja erro específico
    return (
      <div className="max-w-xl mx-auto mt-10 text-white text-center">
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 text-white">
      <div className="flex flex-col items-center p-8 bg-gray-800 rounded-xl shadow-lg text-center">
        <h3 className="text-3xl font-bold mb-6 text-[#fba931]">
          Editar Produto
        </h3>
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <div>
            <label
              htmlFor="name"
              className="block text-left mb-1 text-[#fba931] font-medium"
            >
          ...
          <div>
            <label
              ...
              className="block text-left mb-1 text-[#fba931] font-medium"
            >
              Valor do Produto <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="value"
            ...
            <textarea
              ...
              value={
                Array.isArray(product.variations)
                  ...
                  : product.variations || ""
              } // Converte array para string, ou mantém string
              onChange={handleChange}
              disabled={isSubmitting}
              required
            ></textarea>
          ...
          <div>
            ...
            {product.imageUrl &&
              !productImage && ( // Mostra botão se há imagem original e nenhuma nova foi selecionada
                <button
                  type="button"
